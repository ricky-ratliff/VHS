...
- The MPV kiosk mode settings are required for this use-case, but the hardware-specific settings must be compatible with your hardware. If you are not using the Acer Chromebox CXI2, verify the media codecs supported by your hardware (gpu) and adjust the MPV configuration accordingly.
  - The [MPV manual](https://mpv.io/manual/master/) has usage examples to get you started quickly with any hardware.
  - This is the MPV configuration applied by the `mpv.conf` file generated by the `vhs-client.sh` script. Verify the included MPV config is compatible with your hardware, or adjust the mpv.conf file portion of the script accordingly:

    ```bash
    # Kiosk mode settings for VHS TV player:
    fullscreen=yes
    loop-playlist=inf
    no-osd-bar
...
- The Dropbox desktop app and daemon have already been installed and signed in to manually. [See instructions.](./configs/dropbox-requirements.md)
  ...
  - All other TV Dropbox folders and the Archive folder have been excluded.
- The `vhs-client.sh` installer script is run without root `(sudo)`.

### Installation Workflow

1. Clone this repo onto your admin/management machine.
2. Apply the [manual configs](./configs/tv-client-config.md) referenced above to each TV client.
3. SSH into the client, create the `~/vhs-client.sh` file and copy the full script into it, then apply execute permissions for your user:

   ```bash
   # create the file
   user@TV-XX:~$ touch ./vhs-client.sh
   user@TV-XX:~$ nano ./vhs-client.sh
   # copy and paste the script into your new file, save and close
   user@TV-XX:~$ chmod +x ./vhs-client.sh
   # verify file permissions include execute for your user
   user@TV-XX:~$ ls -lah ./vhs-client.sh
   -rwxrwxr-x 1 user user 4.8K Jun 13 13:43 ./vhs-client.sh
   ```
...
```bash
user@TV-XX:~$ :~$ systemctl --user status tv-02-watch-playlist.service 
● tv-02-watch-playlist.service - TV-02 Dropbox playlist watcher
     Loaded: loaded (/home/user/.config/systemd/user/tv-02-watch-playlist.service; enabled; preset: enabled)
 ...
 Invocation: 723713e58ab54fa6924460aaea152ef3
   Main PID: 937 (bash)
     ...
     CGroup: /user.slice/user-1000.slice/user@1000.service/app.slice/tv-02-watch-playlist.service
             ├─ 937 bash /home/user/bin/TV-02/watch-playlist.sh
             ├─1711 inotifywait -m -e create -e delete -e moved_to -e moved_from -e close_write /home/user/Dropbox/VHS/TV-02/Now-Playing/media
             └─1712 bash /home/user/bin/TV-02/watch-playlist.sh
